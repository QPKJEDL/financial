...
class OnOrderController extends Controller
{
    ...
    public function index(Request $request){
        ...
        return view('onAgent.order.list',['list'=>$data,'desk'=>$this->getDeskList(),'curr'=>$curr,'game'=>Game::getGameByType(),'input'=>$request->all(),'min'=>$min,'pages'=>ceil(count($count)/10)]);
    }
    ...
    public function getOrderListByUserId($id,$begin,$end,Request $request){
        ...
        if ($sql!='' || $sql!=null){
            ...
            return view('onAgent.order.list',['list'=>$data,'desk'=>$this->getDeskList(),'curr'=>$curr,'game'=>Game::getGameByType(),'input'=>$request->all(),'min'=>config('admin.min_date'),'pages'=>ceil(count($count)/10)]);
        }else{
            $data=array();
            return view('onAgent.order.list',['list'=>$data,'desk'=>$this->getDeskList(),'curr'=>$curr,'game'=>Game::getGameByType(),'input'=>$request->all(),'min'=>config('admin.min_date'),'pages'=>1]);
        }
    }
